...
model_path = '../../Modeling/checkpoints/'
models = sorted([folder for folder in os.listdir(model_path) if folder.endswith('pt')])
...
for model in models:
    ...
    logger.info(f'Initating model {model} for training')
    model = BERTClass(model_path=os.path.join(model_path, model))
